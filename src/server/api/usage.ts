...
// Returns true if the output looks like a "please upgrade" error rather than valid data
function isVersionUpgradeError(result: { stdout: string; stderr: string }): boolean {
  const text = result.stdout + result.stderr;
  return /update|upgrade|new version|out.?of.?date|outdated/i.test(text);
}

// Run ccusage, and if we get a version-upgrade error retry with @latest automatically
async function runCcusageAutoUpdate(args: string[]): Promise<{ stdout: string; stderr: string }> {
  const result = await runCcusage(args);
  // If stdout doesn't look like JSON and we see upgrade language, retry with @latest
  const stdout = result.stdout.trim();
  if ((!stdout.startsWith('{') && !stdout.startsWith('[')) && isVersionUpgradeError(result)) {
    const latestArgs = args.map(a => a.replace(/@\d+$/, '@latest'));
    return runCcusage(latestArgs);
  }
  return result;
}
...
async function fetchUsageCached(since: string): Promise<any> {
  ...
  // If another request for the same date range is already fetching, reuse it
  if (_inFlight.has(since)) return _inFlight.get(since)!;

  const promise = (async () => {
    try {
      const [claudeResult, codexResult] = await Promise.all([
        runCcusageAutoUpdate(['ccusage@17', 'daily', '--since', since, '--json']),
        runCcusageAutoUpdate(['@ccusage/codex@17', 'daily', '--since', since, '--json']),
      ]);
      ...
      _inFlight.delete(since);
    }
  })();

  _inFlight.set(since, promise);
  return promise;
}
