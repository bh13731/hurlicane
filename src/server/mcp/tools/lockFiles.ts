...
export const lockFilesSchema = z.object({
  ...
  timeout_ms: z.number().optional().describe('How long to wait for the lock in ms before giving up (default: 120000)'),
});

export async function lockFilesHandler(agentId: string, input: z.infer<typeof lockFilesSchema>): Promise<string> {
  const { files, reason, ttl_ms = 600000, timeout_ms = 120000 } = input;
