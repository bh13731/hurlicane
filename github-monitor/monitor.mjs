...
// Read repos and author from .local.config.json
function readLocalConfig() {
  try {
    return JSON.parse(readFileSync(join(__dirname, '..', '.local.config.json'), 'utf8'));
  } catch {
    return {};
  }
}
const localConfig = readLocalConfig();
const REPOS = localConfig.githubMonitorRepos ?? [];
const AUTHOR = localConfig.githubMonitorAuthor ?? '';
...
async function poll() {
  if (!AUTHOR) {
    console.error('githubMonitorAuthor not set in .local.config.json — skipping poll.');
    return;
  }
  if (REPOS.length === 0) {
    console.error('githubMonitorRepos not set in .local.config.json — skipping poll.');
    return;
  }
